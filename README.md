...
After the fixture run writes `council-report.json`, score it against the
...
# The default PR workflow prefers OpenAI and falls back to Gemini:
...
The local `.council.toml` scaffold remains provider-configurable. The default
GitHub Actions PR workflow generated by `council init` prefers
`OPENAI_API_KEY` when present and otherwise falls back to `GOOGLE_API_KEY`.
It writes a temporary CI config for the selected provider. The Gemini fallback
uses `gemini/gemini-2.5-flash`, `reviewer_timeout_seconds = 360`, and
